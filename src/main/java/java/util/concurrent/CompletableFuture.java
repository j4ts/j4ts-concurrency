...
public class CompletableFuture<T> implements CompletionStage<T>, Future<T> {
    ...
    */
    ...
    public <U, V> CompletionStage<V> thenCombine(CompletionStage<U> completionStage, BiFunction<T, U, V> biFunction) {
        ...
        return new CompletableFuture<>(Promise.all(Lang.<Promise.IterablePromiseLikeT<Object>>any(new PromiseLike[]{this.getPromise(), completionStage.toCompletableFuture().getPromise()}))
    ...
    */
    ...
    public <U> CompletionStage<U> applyToEither(CompletionStage<T> completionStage, Function<T, U> transform) {
        this.dependent += 1;
        completionStage.toCompletableFuture().dependent += 1;
        return new CompletableFuture<>(Promise.<T>race(Lang.<Promise.IterablePromiseLikeT<T>>any(new PromiseLike[]{this.getPromise(), completionStage.toCompletableFuture().getPromise()}))
                .then(transform));
    }
    ...
    */
    ...
    public CompletionStage<Void> acceptEitherAsync(CompletionStage<T> completionStage, Consumer<T> consumer) {
        this.dependent += 1;
        completionStage.toCompletableFuture().dependent += 1;
        return new CompletableFuture<Void>(Promise.<T>race(Lang.<Promise.IterablePromiseLikeT<T>>any(new PromiseLike[]{this.getPromise(), completionStage.toCompletableFuture().getPromise()}))
                .thenAsync((T t) -> {
                    consumer.accept(t);
                    return Promise.resolve();
                }));
    ...
    */
    ...
    public CompletionStage<Void> runAfterEither(CompletionStage<?> completionStage, Runnable runnable) {
        this.dependent += 1;
        completionStage.toCompletableFuture().dependent += 1;
        return new CompletableFuture<>(Promise.race(Lang.<Promise.IterablePromiseLikeT<Object>>any(new PromiseLike[]{this.getPromise(), completionStage.toCompletableFuture().getPromise()}))
                .then(runnable));
    }
    ...
    */
    ...
    public <U> CompletionStage<U> thenCompose(Function<T, CompletionStage<U>> function) {
        return new CompletableFuture<U>(getPromise().thenAsync((T t) -> function.apply(t).toCompletableFuture().getPromise()));
    }
